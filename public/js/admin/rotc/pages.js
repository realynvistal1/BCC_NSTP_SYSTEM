async function adminPage(_role, page, content) {
  ...
  if (page === 'dashboard') {
    ...
    if (intro) {
      intro.innerHTML = `
        ...
        <p>${program === 'ROTC'
          ? 'Track cadet movement, assignment load, approvals, and ROTC actions from one control center.'
          : 'Monitor company allocation, enrollment flow, and CWTS readiness from one overview.'}
        </p>
      `;
    }
    ...
    content.innerHTML = `
      ...
      <section class="section-card distribution-card">
        <div class="section-heading">
          ...
          <p>Approved students in each group for the current enrollment cycle.</p>
        </div>
        <div class="distribution-insights rotc-group-counts">
          ${insight('Battalion 1', Number(data.distribution?.battalion_1 || 0), 'Students in Battalion 1')}
          ${insight('Battalion 2', Number(data.distribution?.battalion_2 || 0), 'Students in Battalion 2')}
          ${insight('Advance Course', Number(data.distribution?.advance_course || 0), 'Students in Advance Course')}
          ${insight('Special Platoons', Number(data.distribution?.special_platoons || 0), 'Students across all special platoons')}
        </div>
      </section>
      <section class="section-card">
        <div class="section-heading">
          <h2>Management Shortcuts</h2>
          <p>Keep the most important admin pages one click away.</p>
        </div>
        <div class="portal-dashboard-grid">
          ${dashCard('Enrollment Schedule', 'Manage Schedule', `Manage the ${program} enrollment period.`, `/admin/${programKey}/enrollment-schedule`, 'schedule', 'blue')}
          ${dashCard('Enrollment List', `${total} Students`, 'View and manage all enrollment applications.', `/admin/${programKey}/enrollment-list`, 'enrollment', 'blue')}
          ${dashCard(program === 'ROTC' ? 'Platoon List' : 'Company List', `${assigned} Assigned`, program === 'ROTC' ? 'Manage ROTC platoon assignments.' : 'View CWTS company assignments.', `/admin/${programKey}/${program === 'ROTC' ? 'platoon-roster' : 'company-roster'}`, 'platoon', 'green')}
          ${dashCard('Attendance Summary', 'View Attendance', 'Tap to view daily attendance records.', `/admin/${programKey}/attendance-summary`, 'attendance', 'cyan')}
          ${dashCard('View Records', 'Student Records', 'Tap to view complete student records.', `/admin/${programKey}/view-records`, 'records', 'indigo')}
    ...
    content.innerHTML = `
      <div class="panel">
        <div class="panel-head">
          <div>
            <h2>Encode Grades</h2>
            <p class="panel-subtitle">Enter midterm and final grades using the 1.00-5.00 college grading scale. A final grade from 1.00 to 3.00 is Passed.</p>
          </div>
        </div>
        ${table(
          ...
          rows.map((row) => `
            <tr>
              ...
              <td><input id="f${row.student_id}" type="number" min="1" max="5" step="0.01" value="${row.final_term ?? ''}"></td>
              <td>${row.grade ?? '-'}</td>
              <td>${row.status ? badge(row.status) : '-'}</td>
              <td><button class="btn small primary" onclick="saveGrade('${programKey}',${row.student_id})">Save</button></td>
            </tr>
          `)
        )}
      </div>
      ...
      <div class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-subtitle">Track absences/offenses and settlement status.</p>
          </div>
        </div>
        ${table(
          ['Student', 'Offenses', 'Settled', 'Action'],
          rows.map((row) => `
            <tr>
              <td><strong>${esc(row.student_no)}</strong><br>${esc(`${row.last_name}, ${row.first_name}`)}</td>
              <td><input id="o${row.student_id}" type="number" min="0" value="${row.offend}"></td>
              <td><input id="s${row.student_id}" type="checkbox" ${row.settled ? 'checked' : ''} style="width:auto"></td>
              <td><button class="btn small primary" onclick="saveOffense('${programKey}',${row.student_id})">Save</button></td>
            </tr>
          `)
        )}
      </div>
      ...
      <div class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-subtitle">Issue official NSTP serial numbers to qualified students.</p>
          </div>
        </div>
        ${table(
          ['Student', 'Serial Number', 'Action'],
          rows.map((row) => `
            <tr>
              <td><strong>${esc(row.student_no)}</strong><br>${esc(`${row.last_name}, ${row.first_name}`)}</td>
              <td><input id="sn${row.student_id}" value="${esc(row.serial_number || '')}" placeholder="NSTP-2026-0001"></td>
              <td><button class="btn small primary" onclick="saveSerial('${programKey}',${row.student_id})">Save</button></td>
            </tr>
          `)
        )}
      </div>
      ...
      <div class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-subtitle">Consolidated ${programKey.toUpperCase()} student information.</p>
          </div>
        </div>
        ${recordsTable(rows)}
      </div>
  ...
  if (page === 'attendance-summary') {
    ...
    const query = ['overall', 'attendance-summary'].includes(group)
      ? ''
    ...
    content.innerHTML = `
      <div class="panel">
        <div class="panel-head">
          <div>
            <h2>${programKey.toUpperCase()} Attendance Summary</h2>
            <p class="panel-subtitle">Aggregated present, late, and absent records per student.</p>
          </div>
        </div>
        ${table(
          ['Student', 'Assignment', 'Present', 'Late', 'Absent', 'Total'],
          rows.map((row) => `
            <tr>
              <td><strong>${esc(row.student_id)}</strong><br>${esc(`${row.last_name}, ${row.first_name}`)}</td>
              <td>${esc(row.assignment || '-')}</td>
              <td>${row.present || 0}</td>
              <td>${row.late || 0}</td>
              <td>${row.absent || 0}</td>
...
async function setEnrollment(programKey, id, status) {
  ...
  try {
    const result = await API.patch(`/api/admin/${programKey}/enrollments/${id}`, {
      status,
...
async function saveGrade(programKey, id) {
  try {
    ...
    if (midterm < 1 || midterm > 5 || finalTerm < 1 || finalTerm > 5) {
      return toast('Grades must be between 1.00 and 5.00.', true);
    }

    const result = await API.post(`/api/admin/${programKey}/grades`, {
      student_id: id,
      ms_level: msLevel,
      midterm,
...
async function saveOffense(programKey, id) {
  try {
    const result = await API.post(`/api/admin/${programKey}/offenses`, {
      student_id: id,
      offend: $(`#o${id}`).value,
      settled: $(`#s${id}`).checked,
    });
    toast(result.message);
  } catch (error) {
    toast(error.message, true);
...
async function saveSerial(programKey, id) {
  try {
    ...
    if (!value) {
      return toast('Enter a serial number first.', true);
    }

    const result = await API.post(`/api/admin/${programKey}/serial-numbers`, {
      student_id: id,
      serial_number: value,
    });
    toast(result.message);
...
function recordsTable(rows) {
  return table(
    ['Student ID', 'Name', 'Course', 'Year', 'Program', 'Assignment', 'Email'],
    rows.map((row) => `
      <tr>
        <td><strong>${esc(row.student_id)}</strong></td>
        <td>${esc(`${row.last_name}, ${row.first_name}`)}</td>
        <td>${esc(row.course)}</td>
        <td>${esc(row.year_level)}</td>
        <td>${esc(row.nstp_component)}</td>
        <td>${esc(row.company || row.special_unit || row.rotc_company || '-')}</td>
        <td>${esc(row.email)}</td>
      </tr>
    `)
  );
