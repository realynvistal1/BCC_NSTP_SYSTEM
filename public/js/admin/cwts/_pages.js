async function adminPage(role,page,c){
  ...
  if(page==='dashboard'){
    ...
    if(intro){
      ...
      c.innerHTML=`<div class="summary-grid three">${summaryTile('Approved',approved,`${
        approvalRate
      }
    % of students cleared`,'green')}${summaryTile('Pending',pending,`${
      pendingRate
    }
    % still need review`,'orange')}${summaryTile('Rejected Students',rejected,'Students who were not approved','red')}</div><section class="section-card distribution-card"><div class="section-heading"><h2>${program} Distribution</h2><p>${program==='ROTC'?'Current view highlights approved cadets and assignment readiness.':'Company analytics show how students are spread across available CWTS units.'}</p></div><div class="distribution-insights">${insight(program==='ROTC'?'Assigned Cadets':'Assigned Students',assigned,`Out of ${
      approved
    }
    approved students`)}${insight(program==='ROTC'?'Needs Assignment':'Needs Company',Math.max(approved-assigned,0),'Approved students still waiting for placement')}${insight('Assignment Rate',approved?Math.round((assigned/approved)*100)+'%':'0%','Current approved-student assignment coverage')}</div></section><section class="section-card"><div class="section-heading"><h2>Management Shortcuts</h2><p>Keep the most important admin pages one click away.</p></div><div class="portal-dashboard-grid">${dashCard('Enrollment Schedule','Manage Schedule',`Manage the ${
      program
    }
...
async function saveGrade(p,id){
  try{
    ...
    toast(`${x.message} Final Grade: ${Number(x.grade).toFixed(2)} â€¢ ${x.status}`);
    setTimeout(()=>location.reload(),500)
  }   catch(e){
    toast(e.message,true)
  }
