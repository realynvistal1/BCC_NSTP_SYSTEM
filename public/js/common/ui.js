...
async function renderEnrollmentList(p,c){
  ...
  ['enrollmentSearch','filterLevel','filterSY','filterYear','filterCourse','filterMedical'].forEach(id=>$('#'+id).addEventListener(id==='enrollmentSearch'?'input':'change',draw));
...
function bindRosterExpanders(){
  $$('[data-expand]').forEach(b=>b.onclick=()=>{const key=b.dataset.expand;const card=$(`[data-expand-card="${key}"]`);card?.classList.toggle('open')})
}
