...
async function renderAdminRecords(program,content){
  const p=program.toLowerCase(),prefix=program==='CWTS'?'CWTS':'MS';
  ...
  content.innerHTML=`
    ...
    <section class="panel record-list-panel"><div class="table-wrap"><table class="old-table"><thead><tr><th>#</th><th>Student ID</th><th>Name</th><th>Course</th><th>${prefix} Level</th><th>SY</th>${program==='ROTC'?'<th>Battalion / Group</th>':''}<th>Action</th></tr></thead><tbody id="recordRows"></tbody></table></div><div class="table-footer" id="recordFooter"></div></section>
    <div class="old-modal hidden" id="recordModal"><div class="old-modal-backdrop"></div><div class="old-modal-card record-modal-card"><div id="recordModalBody"></div></div></div>`;
  ...
  async function openRecord(id,level){
    ...
    try{
      ...
      body.innerHTML=`<div class="record-modal-head"><div><span>Student Record — ${prefix} ${esc(level)}</span><h2>${esc(s.first_name)} ${esc(s.last_name)}${s.suffix?` ${esc(s.suffix)}`:''}</h2><p>${c.school_year?`SY ${esc(c.school_year)} · `:''}${esc(s.student_id)}</p></div><button class="modal-close" id="recordClose">×</button></div><div class="record-modal-scroll">
      ...
      </div><div class="old-modal-actions"><button class="btn" id="recordCloseBottom">Close</button></div>`;
      const close=()=>modal.classList.add('hidden'); $('#recordClose').onclick=close; $('#recordCloseBottom').onclick=close; modal.querySelector('.old-modal-backdrop').onclick=close;
