...
async function renderAdminGrades(program, content){
  ...
  content.innerHTML=`
    ...
    <section class="panel grade-panel">
      ...
      <div class="table-wrap"><table class="old-table grade-table"><thead><tr><th>#</th><th>Student ID</th><th>Student Name</th><th>Course & Year</th><th>Grade Status</th><th>Action</th></tr></thead><tbody id="gradeRows"></tbody></table></div>
      <div class="table-footer" id="gradeFooter"></div>
    </section>
    <div id="gradeModal" class="old-modal hidden"><div class="old-modal-backdrop"></div><div class="old-modal-card grade-modal-card"><div id="gradeModalBody"></div></div></div>`;
  ...
  function openModal(s){
    ...
    $('#gradeModalBody').innerHTML=`<div class="old-modal-head"><div class="grade-student-head"><div class="grade-avatar">${esc((s.first_name||'?')[0])}${esc((s.last_name||'?')[0])}</div><div><small>${esc(s.student_no)}</small><h3>${esc(s.last_name)}, ${esc(s.first_name)}</h3><p>${esc(s.course)} · ${esc(s.year_level)}</p></div></div><button class="modal-close" id="closeGradeModal">×</button></div><div class="grade-modal-content">${section(s,'1')}${section(s,'2')}<div id="gradeMessage"></div></div><div class="old-modal-actions"><button class="btn" id="cancelGrade">Close</button><button class="btn primary" id="saveGrades">Save Grades</button></div>`;
    modal.classList.remove('hidden');
    const close=()=>modal.classList.add('hidden'); $('#closeGradeModal').onclick=close; $('#cancelGrade').onclick=close; modal.querySelector('.old-modal-backdrop').onclick=close;
